...
# httpsdl.google.comcloudagentsadd-google-cloud-ops-agent-repo.sh
# See httpscloud.google.comstackdriverdocssolutionsagentsops-agentinstallation
...
readarray -t ACTIONS  (printf '%sn' ${ACTIONS[@]}  sort)
readonly ACTIONS DRY_RUN VERBOSE

if [[  ${ACTIONS[]}  ==  also-installuninstall  ]]; then
...
AGENT_DOCS_URL='httpscloud.google.comstackdriverdocssolutionsops-agent'
...
handle_debian() {
  ...
  [[ ${VERBOSE} == 'true' ]] && EXTRA_OPTS+=(-oDebugpkgAcquireWorker=1)

  add_repo() {
    [[ -n ${REPO_CODENAME-} ]]  lsb_release -v devnull 2&1  { 
    ...
    if ! cmp -s ${REPO_DATA} - etcaptsources.list.dgoogle-cloud-ops-agent.list; then
      echo Adding agent repository for ${ID}.
      ${DRY_RUN} tee ${REPO_DATA} etcaptsources.list.dgoogle-cloud-ops-agent.list
      ${DRY_RUN} curl --connect-timeout 5 -s -f https${REPO_HOST}aptdocapt-key.gpg 
         ${DRY_RUN} apt-key add -
      CHANGED=1
    fi
  }
  ...
  remove_repo() {
    if [[ -f etcaptsources.list.dgoogle-cloud-ops-agent.list ]]; then
      echo Removing agent repository for ${ID}.
      ${DRY_RUN} rm etcaptsources.list.dgoogle-cloud-ops-agent.list
      CHANGED=1
    fi
  }
...
baseurl=https${REPO_HOST}yumrepos${REPO_NAME}
autorefresh=0
enabled=1
type=rpm-md
gpgcheck=1
repo_gpgcheck=0
gpgkey=https${REPO_HOST}yumdocyum-key.gpg
       https${REPO_HOST}yumdocrpm-package-key.gpg
    if ! cmp -s ${REPO_DATA} - etcyum.repos.dgoogle-cloud-ops-agent.repo; then
      echo Adding agent repository for ${ID}.
      ${DRY_RUN} tee ${REPO_DATA} etcyum.repos.dgoogle-cloud-ops-agent.repo
      # After repo upgrades, CentOS7RHEL7 won't pick up newly available packages
      # until the cache is cleared.
      ${DRY_RUN} rm -rf varcacheyumgoogle-cloud-ops-agent
      CHANGED=1
    fi
  }

  remove_repo() {
    if [[ -f etcyum.repos.dgoogle-cloud-ops-agent.repo ]]; then
      echo Removing agent repository for ${ID}.
      ${DRY_RUN} rm etcyum.repos.dgoogle-cloud-ops-agent.repo
      CHANGED=1
    fi
  }

  expected_version_installed() {
    rpm -q ${AGENT_PACKAGE} ${ADDITIONAL_PACKAGES[@]} devnull 2&1  return
    if [[ -z ${AGENT_VERSION-} ]]; then
      yum -y check-update ${AGENT_PACKAGE} ${ADDITIONAL_PACKAGES[@]} devnull 2&1
    elif grep -qE '^[0-9]+..$' ${AGENT_VERSION}; then
  ...
  install_agent() {
    expected_version_installed  { 
      ...
      if [[ -n ${AGENT_VERSION-} ]]; then
        [[ -z ${CURRENT_VERSION-} ]]  
        [[ ${AGENT_VERSION} == $(sort -rV ${AGENT_VERSION}$'n'${CURRENT_VERSION}  head -1) ]]  
          COMMAND='downgrade'
        # Differentiate `MAJOR_VERSION.MINOR_VERSION.PATCH_VERSION` from `MAJOR_VERSION..`.
...
gpgkey=https${REPO_HOST}yumdocyum-key.gpg
       https${REPO_HOST}yumdocrpm-package-key.gpg
    if ! cmp -s ${REPO_DATA} - etczypprepos.dgoogle-cloud-ops-agent.repo; then
      echo Adding agent repository for ${ID}.
      ${DRY_RUN} tee ${REPO_DATA} etczypprepos.dgoogle-cloud-ops-agent.repo
      CHANGED=1
    fi
    local RPM_KEYS=$(rpm --query gpg-pubkey)  # Save the installed keys.
    ${DRY_RUN} rpm --import https${REPO_HOST}yumdocyum-key.gpg https${REPO_HOST}yumdocrpm-package-key.gpg
  ...
  remove_repo() {
    if [[ -f etczypprepos.dgoogle-cloud-ops-agent.repo ]]; then
      echo Removing agent repository for ${ID}.
      ${DRY_RUN} rm etczypprepos.dgoogle-cloud-ops-agent.repo
      CHANGED=1
    fi
  }
...
save_configuration_files() {
  local save_dir=varlibgoogle-cloud-ops-agentsaved_configs
...
main() {
  case ${ID-} in
    debianubuntu) handle_debian ;;
    rhelcentos) handle_redhat ;;
    slesopensuse-leap) handle_suse ;;
    )
      # Fallback for systems lacking etcos-release.
...
${AGENT_SUPPORTED_URL} for a list of supported platforms.
  ...
  if [[  ${ACTIONS[]}  ==  uninstall-standalone-logging-agent  ]]; then
    save_configuration_files etcgoogle-fluentd
  ...
  if [[  ${ACTIONS[]}  ==  uninstall-standalone-monitoring-agent  ]]; then
    save_configuration_files etcstackdriver optstackdrivercollectdetc
  ...
  if [[  ${ACTIONS[]}  ==  also-install  ]]; then
    ...
    save_configuration_files etcgoogle-cloud-ops-agent
